...
psychoJS.start({
  ...
  resources: [
    // resources:
    {'name': 'conditions3.csv', 'path': 'conditions3.csv'},
    {'name': 'conditions3.csv', 'path': 'conditions3.csv'},
...
function visual_searchRoutineBegin(snapshot) {
  return async function () {
    ...
    for (var i, _pj_c = 0, _pj_a = util.range(n_distractors1), _pj_b = _pj_a.length; (_pj_c < _pj_b); _pj_c += 1) {
        i = _pj_a[_pj_c];
        distractor1 = new visual.Polygon({"win": psychoJS.window, "edges": 4, "size": 0.05, "pos": [xys[count][0], xys[count][1]], "lineColor": [(- 0.4902), 0.7647, 0.6314], "fillColor": [(- 0.4902), 0.7647, 0.6314], "opacity": 1.0, "depth": (- 1.0), "ori": 45});
    ...
    for (var i, _pj_c = 0, _pj_a = util.range(n_distractors2), _pj_b = _pj_a.length; (_pj_c < _pj_b); _pj_c += 1) {
        i = _pj_a[_pj_c];
        distractor2 = new visual.Polygon({"win": psychoJS.window, "edges": 1000, "size": 0.04, "pos": [xys[count][0], xys[count][1]], "lineColor": [1.0, 0.5059, 0.5922], "fillColor": [1.0, 0.5059, 0.5922], "opacity": 1.0, "depth": (- 1.0), "ori": 45});
    ...
    instruct_t.setText('มีวงกลมสีฟ้าอมเขียว      หรือไม่');
    ...
    if (mouse_t.status === PsychoJS.Status.STARTED) {  // only update if started and not finished!
      ...
      if (!_mouseButtons.every( (e,i,) => (e == prevButtonState[i]) )) { // button state changed?
        ...
        if (_mouseButtons.reduce( (e, acc) => (e+acc) ) > 0) { // state changed to a new click
          ...
          gotValidClick = false;
          mouse_t.clickableObjects = eval([target_3, C_3, InC_3])
...
function visual_searchRoutineEnd(snapshot) {
  return async function () {
    ...
    if ((mouse_t.clicked_name.slice((- 1))[0] === corrAns_3)) {
        correct = 1;
    } else {
        correct = 0;
    }
...
function visual_search_3RoutineBegin(snapshot) {
  return async function () {
    ...
    visual_search_3Components.forEach( function(thisComponent) {
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
       });
    return Scheduler.Event.NEXT;
...
function endRoutineBegin(snapshot) {
  return async function () {
    ...
    fetch('https://pipe.jspsych.org/api/data', {
        ...
        body: JSON.stringify({
            experimentID: 'Cpx5UFMaIxuR', // ⭑ UPDATE WITH YOUR DATAPIPE EXPERIMENT ID ⭑
            filename: filename,
            data: data,
        }),
